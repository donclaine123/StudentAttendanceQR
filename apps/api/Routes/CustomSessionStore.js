  ...
  async touch(sid, session, callback) {
    ...
    try {
      ...
      const [sessionRows] = await conn.query(
        'SELECT expires_at FROM sessions WHERE session_id = ? AND is_active = TRUE',
        [sid]
      );
      
      // If session doesn't exist or is already expired, don't update it
      if (sessionRows.length === 0) {
        if (shouldLog) {
          console.log("❌ Session not found or inactive, skipping touch");
        }
        return callback(null);
      }
      
      const nowDate = new Date();
      const expiryDate = new Date(sessionRows[0].expires_at);
      
      // If session is already expired, don't extend it
      if (expiryDate <= nowDate) {
        if (shouldLog) {
          console.log("❌ Session already expired, skipping touch");
        }
        return callback(null);
      }
      
      // Only update last_activity, NOT expires_at
      await conn.query(
        'UPDATE sessions SET last_activity = NOW() WHERE session_id = ? AND is_active = TRUE',
        [sid]
      );
